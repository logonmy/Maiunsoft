 ...
 */
namespace Common\Tool\PHPMailer;
/**
 * PHPMailer - PHP email creation and transport class.
 * @package PHPMailer
 * @author Marcus Bointon (Synchro/coolbru) <phpmailer@synchromedia.co.uk>
...
class PHPMailer {
	...
	protected function addAnAddress($kind, $address, $name = '') {
		if (!preg_match('/^(to|cc|bcc|Reply-To)$/', $kind)) {
			E('Invalid recipient array: '.$kind);
			return false;
		}
	...
	public function setFrom($address, $name = '', $auto = true) {
		...
		if (!$this->validateAddress($address)) {
			...
			if ($this->exceptions) {
				E($this->lang('invalid_address').': '.$address);
			}
			return false;
		}
		$this->From = $address;
		...
		if ($this->SingleTo === true) {
			foreach ($this->SingleToArray as $toAddr) {
				if (!@$mail = popen($sendmail, 'w')) {
					E($this->lang('execute').$this->Sendmail);
				}
	...
	public function getSMTPInstance() {
		if (!is_object($this->smtp)) {
			$this->smtp = new \Common\Tool\PHPMailer\SMTP;
		}
		return $this->smtp;
	}
	...
	public function setLanguage($langcode = 'en', $lang_path = '') {
		...
		$PHPMAILER_LANG = array(
			...
			'provide_address' => '脉云：你必须提供至少一个收件人邮箱.',
	...
	public function createBody() {
		...
		if ($this->isError()) {
			...
			if (!defined('PKCS7_TEXT')) {
				E($this->lang('signing').' OpenSSL extension missing.');
			}
			...
			if (@openssl_pkcs7_sign(
				...
				@unlink($file);
				@unlink($signed);
				E($this->lang('signing').openssl_error_string());
			}
		}
		return $body;
	}
	...
	public function addAttachment($path, $name = '', $encoding = 'base64', $type = '', $disposition = 'attachment') {
		if (!@is_file($path)) {
			E($this->lang('file_access').$path, self::STOP_CONTINUE);
		}
	...
	protected function encodeFile($path, $encoding = 'base64') {
		if (!is_readable($path)) {
			E($this->lang('file_open').$path, self::STOP_CONTINUE);
		}
