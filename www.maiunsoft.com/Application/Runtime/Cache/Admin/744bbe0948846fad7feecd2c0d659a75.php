...
<form role="form" action="<?php echo U('article/save_content');?>" method="post">
    <section class="">
        <textarea name="article_content" id="product_description" style="width:100%;height:500px"><?php echo (htmlspecialchars_decode($article["content"])); ?></textarea>
    </section>
    <div class="col-lg-12" style="text-align:center;margin-top:10px">
        <input type="hidden" name="article_group" value="<?php echo ($article["group_id"]); ?>" />
        <button type="submit" class="btn-danger btn" name="article_id" value="<?php echo ($article["id"]); ?>">保存内容</button>
    </div>
</form>
...
$(document).ready(function(){
    $("#productSelector").change(function(){
        var url = "<?php echo U('product/description');?>?id=" + $("#productSelector").val();
        window.location.href = url;
    });
    var ue = UE.getEditor('product_description');
});
</script>
<script type="text/javascript" src="/www.maiunsoft.com/Public/Static/ueditor/ueditor.config.js"></script>
<script type="text/javascript" src="/www.maiunsoft.com/Public/Static/ueditor/ueditor.all.min.js"></script>
                        <!-- page end-->
                    </div>
                </section>
            </section>
